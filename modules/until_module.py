...
class CrossEn(nn.Module):
    ...
    def forward(self, sim_matrix):
        logpt = F.log_softmax(sim_matrix + 1e-10, dim=-1)
        logpt = torch.diag(logpt)
        nce_loss = -logpt
        sim_loss = nce_loss.mean()
        return sim_loss

class CrossEn_KL(nn.Module):
    def __init__(self,):
        super(CrossEn_KL, self).__init__()
    ...
    def forward(self, sim_matrix):
        ...
        print('2222222222222222222 pos_loss:',pos_loss)
        neg_loss = -((1 - label) * (torch.log(1 - sims))).mean()
        print('2222222222222222222 neg_loss:',neg_loss)
        #print('pos_loss,negloss:',pos_loss, neg_loss)
        sim_loss = pos_loss + neg_loss
        print('2222222222222222222 sim_loss:',sim_loss)
        return sim_loss
